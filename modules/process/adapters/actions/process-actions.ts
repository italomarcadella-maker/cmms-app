...
import { PrismaProjectRepository } from '../db/prisma-project-repository';
import { PrismaSopDocumentRepository } from '../db/prisma-sop-document-repository';
import { PrismaProcessAnomalyRepository } from '../db/prisma-process-anomaly-repository';
...
try {
    ServiceLocator.resolve('IProjectRepository');
} catch {
    ServiceLocator.register('IProjectRepository', new PrismaProjectRepository());
}
try {
    ServiceLocator.resolve('ISopDocumentRepository');
} catch {
    ServiceLocator.register('ISopDocumentRepository', new PrismaSopDocumentRepository());
}
try {
    ServiceLocator.resolve('IProcessAnomalyRepository');
} catch {
    ServiceLocator.register('IProcessAnomalyRepository', new PrismaProcessAnomalyRepository());
}

const getProjectRepo = () => ServiceLocator.resolve<PrismaProjectRepository>('IProjectRepository');
const getSopRepo = () => ServiceLocator.resolve<PrismaSopDocumentRepository>('ISopDocumentRepository');
const getAnomalyRepo = () => ServiceLocator.resolve<PrismaProcessAnomalyRepository>('IProcessAnomalyRepository');
...
export async function getUnresolvedAnomalies() {
    try {
        const repo = getAnomalyRepo();
        // Since we need to include asset: true, we query directly using prisma in the adapter or resolve via repo
        // Let's use prisma directly for the combined query, which is standard for database queries needing specific joins
...
export async function getProjects(showArchived: boolean = false) {
    try {
        // Query projects and include tasks
...
export async function getLivePresenceData() {
    try {
        ...
        const realOperators = technicians.map((tech) => {
            const activeAssignment = tech.assignments[0];
